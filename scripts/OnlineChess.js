CHESSAPP.onlinePlay = {
  ...
  connect: function (stg, callback) {
    ...
    this.sk.on("chat", { transports: ["websocket"] }, function (data) {
      CHESSAPP.GamePlay.chatMessage(data);
    });
    this.sk.on("partnerDisconnect", { transports: ["websocket"] }, function () {
      CHESSAPP.GamePlay.statusUpdate({ type: "e", msg: "Your partner has left the game" });
    });
    this.sk.on("disconnect", { transports: ["websocket"] }, function () {
      CHESSAPP.GamePlay.statusUpdate({
        type: "e",
        msg: "Internal Server Error. We will be back soon. Sorry for the inconvenience.",
      });
    });
    this.sk.on("matchfound", { transports: ["websocket"] }, function (data) {
      CHESSAPP.GamePlay.statusUpdate({
        type: "fb",
    ...
    this.sk.on("opposing_move", { transports: ["websocket"] }, function (data) {
